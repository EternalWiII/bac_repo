...
/**
 * AuthService handles authentication and user 
 * registration logic in the application.
 * It is responsible for creating authentication
 *  tokens and managing user registration.
 */
@Service
...
public class AuthService {
    ...
    /**
     * Creates an authentication token for the user based 
     * on the provided credentials.
     *
     * @param authRequest the JWT request containing the user's credentials
     * @return a ResponseEntity containing the JWT token if 
    ...
    /**
     * Creates a new user in the system based on the provided 
     * registration details.
     *
     * @param registrationUserDto the registration details for the new user
     * @return a ResponseEntity containing the created user's information 
    ...
    public ResponseEntity<?> createNewUser(
        ...
        if (!registrationUserDto.getPassword().equals(
            registrationUserDto.getConfirmPassword())
        ) {
            return new ResponseEntity<>(
                new ApplicationError(
        ...
        if (customUserDetailsService.findByUsername(
            registrationUserDto.getUsername()).isPresent()
        ) {
            return new ResponseEntity<>(
                new ApplicationError(
                    HttpStatus.BAD_REQUEST.value(), 
        ...
        final User user = customUserDetailsService.createNewUser(
            registrationUserDto
            );
